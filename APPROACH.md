...
Not yet done: setting `SLACK_ADMIN_USER_ID` and `SLACK_APPROVAL_CHANNEL_ID` in Railway/`.env` (both documented in `.env.example`), and the Slack manual walkthrough end-to-end once the pending manifest push (above) is live.
