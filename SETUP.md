...
**Pending:** the manifest already includes `app_home_opened` (bot event), `features.app_home.home_tab_enabled` for the Home tab status dashboard, `files:write` (scope, required by `client.files.uploadV2` in `src/slack/files.ts`), and `im:write` (scope, required by `client.conversations.open` in `src/slack/commands.ts` — used to resolve the requester's DM channel ID before uploading candidate images into it), but still needs `im:history` (scope) and `message.im` (bot event) added for DM-based `/catalog-sync` (see APPROACH.md). Add all of these, then follow the update steps above and reinstall — the reinstall also picks up the Home tab config in the same pass.
